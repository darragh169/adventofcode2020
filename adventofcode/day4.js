const INPUT = `eyr:2028 iyr:2016 byr:1995 ecl:oth
...
const passportKeys = ["ecl", "pid", "eyr", "hcl", "byr", "iyr", "hgt"];
const EYE_COLORS = ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"];

const parseInput = (input) => {
  const splitInput = input.split(/\r?\n\n/);
  return splitInput.map((item) => extractPassportObj(item));
};

const extractPassportObj = (input) => {
  let first = input.split("\n");
  first = first.map((item) => {
    return item.indexOf(" ") ? item.split(" ") : item;
  });
  first = first.flat();
...
const isValidBirthYear = (byr) => {
  return byr.length === 4 && parseInt(byr) >= 1920 && parseInt(byr) <= 2002;
};

const isValidIssueYear = (iyr) => {
  return iyr.length === 4 && parseInt(iyr) >= 2010 && parseInt(iyr) <= 2020;
};

const isValidExpirationYear = (eyr) => {
  return eyr.length === 4 && parseInt(eyr) >= 2020 && parseInt(eyr) <= 2030;
};

const isValidHeight = (hgt) => {
  if (hgt.indexOf("in") < 0 && hgt.indexOf("cm") < 0) {
    return false;
  }
  if (hgt.indexOf("in") > 0) {
    let tempHgt = parseInt(hgt.split("in")[0]);
    return tempHgt >= 59 && tempHgt <= 76;
  }
  if (hgt.indexOf("cm") > 0) {
    let tempHgt = parseInt(hgt.split("in")[0]);
    return tempHgt >= 150 && tempHgt <= 193;
  }
};

const isValidHairColor = (hairColor) => {
  return new RegExp("^#(?:[0-9a-fA-F]{3}){1,2}$").test(hairColor);
};

const isValidEyeColor = (eyeColor) => {
  return EYE_COLORS.includes(eyeColor);
};

const isValidPassportNumber = (passportNumber) => {
  return passportNumber.toString().length === 9;
};

const isValidPassport = (passport) => {
  if (!passport.byr || !isValidBirthYear(passport.byr)) return false;
  if (!passport.iyr || !isValidIssueYear(passport.iyr)) return false;
  if (!passport.eyr || !isValidExpirationYear(passport.eyr)) return false;
  if (!passport.hgt || !isValidHeight(passport.hgt)) return false;
  if (!passport.hcl || !isValidHairColor(passport.hcl)) return false;
  if (!passport.ecl || !isValidEyeColor(passport.ecl)) return false;
  if (!passport.pid || !isValidPassportNumber(passport.pid)) return false;
  return true;
};

const getValidPassportsCount = (passports) => {
...
console.log(getValidPassportsCount(passports));
